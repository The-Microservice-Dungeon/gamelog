...
class KafkaConsumerConfig {
    ...
    public ConsumerFactory<String, MovementEvent> movementEventConsumerFactory() {
        Map<String, Object> props = new HashMap<>();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "reflectoring-movement-event");
        return new DefaultKafkaConsumerFactory<>(props, new StringDeserializer(), new JsonDeserializer<>(MovementEvent.class));
    }

    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, MovementEvent> movementEventKafkaListenerContainerFactory() {
        ConcurrentKafkaListenerContainerFactory<String, MovementEvent> factory = new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(movementEventConsumerFactory());
        return factory;
    }
